...
import { NAV_ITEMS, NAV_CATEGORIES } from '../../utils/constants';
      ...
      >
        ...
        <nav className="p-4 space-y-1 md:hidden">
          {NAV_ITEMS.map((item, idx) => {
            ...
            const showCategory = idx === 0 || item.category !== NAV_ITEMS[idx - 1].category;
            return (
              <div key={item.path}>
                {showCategory && item.category && (
                  <p className="text-[10px] text-text-muted uppercase tracking-wider font-semibold mt-3 mb-1 first:mt-0 px-3">
                    {NAV_CATEGORIES[item.category] ?? item.category}
                  </p>
                )}
                <Link
                  href={item.path}
