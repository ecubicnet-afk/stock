      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          {totalCount > 0 && (
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-xs text-violet-300 font-mono">{progress.toFixed(0)}%</span>
            </div>
          )}
        </div>
        <div className="flex gap-2">
          <Button
            ...
            className={step === 'dreams' ? '!bg-violet-500/20 !text-violet-300 !border-violet-500/50' : ''}
          >
            Step 1: 夢リスト
          </Button>
          <Button
            ...
            className={step === 'board' ? '!bg-violet-500/20 !text-violet-300 !border-violet-500/50' : ''}
          >
            Step 2: ビジョンボード
          </Button>
        </div>
          ...
          <Card>
            ...
            <div className="flex gap-2 mb-4 flex-wrap">
              <select
                ...
                className="bg-bg-primary border border-border rounded-lg px-2 py-2 text-xs text-text-primary focus:outline-none focus:border-violet-500/50"
              >
              ...
              <input
                ...
                placeholder="夢や目標を入力..."
                className="flex-1 min-w-0 bg-bg-primary border border-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder:text-text-secondary/50 focus:outline-none focus:border-violet-500/50"
              />
              <Button variant="secondary" size="sm" onClick={handleAddDream} className="!bg-violet-500/20 !text-violet-300">
                追加
              </Button>
            </div>
            ...
            <div className="flex gap-2 mb-4 flex-wrap">
              <button
                ...
                className={`px-2.5 py-1 text-xs rounded-full transition-colors ${!filterCategory ? 'bg-violet-500/20 text-violet-300' : 'text-text-muted hover:text-text-secondary'}`}
              >
                全て ({totalCount})
              </button>
              {CATEGORIES.map(c => {
                ...
                return (
                  <button
                    ...
                    className={`px-2.5 py-1 text-xs rounded-full transition-colors ${filterCategory === c.value ? 'bg-violet-500/20 text-violet-300' : 'text-text-muted hover:text-text-secondary'}`}
                  >
                    {c.label} ({count})
                  </button>
                );
            ...
            <div className="space-y-2">
              ...
              {filteredDreams.map((dream, idx) => {
                ...
                return (
                  <div
                    key={dream.id}
                    className={`flex items-start gap-3 p-3 rounded-lg border transition-colors ${dream.done ? 'bg-up/5 border-up/20' : 'bg-bg-primary/50 border-border hover:border-violet-500/30'}`}
                  >
                    ...
                    {editingId === dream.id ? (
                      <input
                        ...
                        autoFocus className="flex-1 bg-transparent text-sm text-text-primary focus:outline-none border-b border-violet-500/50"
                      />
                    ) : (
                      <span
                    ...
                    <button onClick={() => deleteDream(dream.id)} className="text-text-secondary/30 hover:text-red-400 transition-colors shrink-0">
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader
              ...
              action={<Button variant="secondary" size="sm" onClick={() => fileInputRef.current?.click()} className="!bg-violet-500/20 !text-violet-300">画像を追加</Button>}
            />
                ...
                {data.images.map((img) => (
                  <div key={img.id} className="break-inside-avoid group relative rounded-lg overflow-hidden border border-border bg-bg-primary">
                    ...
                    <button
                      ...
                      className="absolute top-1.5 right-1.5 w-6 h-6 bg-black/70 rounded-full flex items-center justify-center text-white/70 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
