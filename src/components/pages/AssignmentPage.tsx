  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      ...
      <div className="bg-bg-card shadow-[var(--shadow-card)] border border-border rounded-xl p-5 mb-6">
        ...
        <div className="flex justify-end mt-3">
          <button
            ...
            className="px-5 py-2 bg-amber-50 text-amber-700 rounded-lg text-sm font-medium hover:bg-amber-100 transition-colors"
          >
            提出する
          </button>
        </div>
      ...
      <div className="space-y-3">
        ...
        {assignments.map((a) => (
          <div key={a.id} className="bg-bg-card shadow-[var(--shadow-card)] border border-border rounded-xl p-4">
            {editingId === a.id ? (
              <>
                <input
                  type="text"
                ...
                <div className="flex justify-end gap-2 mt-2">
                  ...
                  <button onClick={saveEdit} className="px-3 py-1.5 text-xs bg-amber-50 text-amber-700 rounded hover:bg-amber-100">保存</button>
                </div>
              </>
            ) : (
              <>
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => startEdit(a.id)} className="text-text-secondary/50 hover:text-amber-700 transition-colors">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                      </svg>
                    </button>
                    <button onClick={() => deleteAssignment(a.id)} className="text-text-secondary/50 hover:text-red-600 transition-colors">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
