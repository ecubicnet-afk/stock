...
const DRAWING_COLORS = ['#111827', '#ef4444', '#22c55e', '#3b82f6', '#f59e0b', '#a855f7'];
const DRAWING_WIDTHS = [1, 2, 4];

const REGION_STYLES: Record<StrategyNoteRegion, { label: string; emoji: string; bg: string }> = {
  jp: { label: '日本', emoji: '🇯🇵', bg: 'bg-blue-50' },
  us: { label: '米国', emoji: '🇺🇸', bg: 'bg-red-50' },
  other: { label: 'その他', emoji: '🌐', bg: 'bg-slate-50' },
};

const DIRECTION_STYLES: Record<StrategyNoteDirection, { label: string; arrow: string; border: string; color: string }> = {
  bullish: { label: '上昇', arrow: '▲', border: 'border-emerald-300', color: 'text-emerald-700' },
  bearish: { label: '下落', arrow: '▼', border: 'border-red-300', color: 'text-red-700' },
  neutral: { label: '中立', arrow: '―', border: 'border-slate-300', color: 'text-slate-600' },
};
...
export function StrategyCanvas({ notes, connections, drawing, onUpdateDrawing, onUpdateNote, onRemoveNote, onAddConnection, onRemoveConnection }: StrategyCanvasProps) {
  ...
  return (
    <div className="flex flex-col flex-1">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-card border-b border-primary/10 flex-shrink-0 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            ...
            {(['bullish', 'bearish', 'neutral'] as const).map((d) => (
              <button key={d} onClick={() => setConnectDirection(d)}
                className={`px-2 py-1 rounded text-xs ${connectDirection === d ? 'ring-1 ring-gray-900' : ''}`}
        ...
        {selectedConnection && (
          <button onClick={() => { onRemoveConnection(selectedConnection); setSelectedConnection(null); }}
            className="px-3 py-1.5 rounded text-xs bg-red-50 text-red-700 hover:bg-red-100">選択した線を削除</button>
        )}
        ...
        <button
          ...
          className={`px-2 py-1.5 rounded text-xs font-medium transition-colors ${drawingTool === 'pen' ? 'bg-amber-100 text-amber-700' : 'bg-primary/10 text-secondary hover:bg-primary/20'}`}
        >
          ✏️ ペン
        </button>
        <button
          ...
          className={`px-2 py-1.5 rounded text-xs font-medium transition-colors ${drawingTool === 'strokeEraser' ? 'bg-amber-100 text-amber-700' : 'bg-primary/10 text-secondary hover:bg-primary/20'}`}
        >
          🧹 消しゴム
        </button>
        <button
          onClick={() => { const next = drawingTool === 'eraser' ? null : 'eraser' as DrawingTool; setDrawingTool(next); if (next) setConnectMode(false); }}
          className={`px-2 py-1.5 rounded text-xs font-medium transition-colors ${drawingTool === 'eraser' ? 'bg-amber-100 text-amber-700' : 'bg-primary/10 text-secondary hover:bg-primary/20'}`}
        >
          ◯ 範囲消し
        </button>
        <button
          onClick={() => { const next = drawingTool === 'text' ? null : 'text' as DrawingTool; setDrawingTool(next); if (next) setConnectMode(false); }}
          className={`px-2 py-1.5 rounded text-xs font-medium transition-colors ${drawingTool === 'text' ? 'bg-amber-100 text-amber-700' : 'bg-primary/10 text-secondary hover:bg-primary/20'}`}
        >
          Aa テキスト
        </button>
            ...
            <div className="flex items-center gap-1">
              {DRAWING_COLORS.map((c) => (
                <button key={c} onClick={() => setDrawingColor(c)}
                  className={`w-5 h-5 rounded-full border-2 ${drawingColor === c ? 'border-gray-900' : 'border-transparent'}`}
              ...
              <div className="flex items-center gap-1">
                {DRAWING_WIDTHS.map((w) => (
                  <button key={w} onClick={() => setDrawingWidth(w)}
                    className={`px-1.5 py-1 rounded text-[10px] ${drawingWidth === w ? 'bg-gray-200 text-gray-900' : 'text-muted hover:text-gray-900'}`}>
                    {w}px
                  </button>
                ))}
              </div>
        ...
        {(drawing.paths.length > 0 || drawing.texts.length > 0) && (
          <button
            ...
            className="px-2 py-1.5 rounded text-xs bg-red-50 text-red-700 hover:bg-red-50"
          >
            全消去
          </button>
        )}
      ...
      {/* Canvas */}
      <div ref={canvasRef} className="flex-1 overflow-auto relative bg-gray-50"
        ...
        <div className="relative" style={{ width: CANVAS_WIDTH, height: CANVAS_HEIGHT }}>
          ...
          <svg className="absolute inset-0 pointer-events-none" width={CANVAS_WIDTH} height={CANVAS_HEIGHT}>
            ...
            {dateColumns.filter((c) => c.isWeekend).map((col, i) => (
              <rect key={`we-${i}`} x={col.x} y={0} width={PX_PER_DAY} height={CANVAS_HEIGHT} fill="rgba(0,0,0,0.02)" />
            ))}
            {dateColumns.length > 0 && <rect x={dateColumns[0].x} y={0} width={PX_PER_DAY} height={CANVAS_HEIGHT} fill="rgba(14,116,144,0.05)" />}
            {priceLines.map((pl) => (
              <g key={pl.price}>
                <line x1={MARGIN_LEFT} y1={pl.y} x2={CANVAS_WIDTH} y2={pl.y} stroke={pl.isMajor ? 'rgba(0,0,0,0.12)' : 'rgba(0,0,0,0.05)'} strokeWidth={pl.isMajor ? 1 : 0.5} />
                <text x={MARGIN_LEFT - 5} y={pl.y + 4} fill={pl.isMajor ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.35)'} fontSize={pl.isMajor ? 13 : 11} textAnchor="end" fontFamily="monospace">{(pl.price / 10000).toFixed(1)}万</text>
              </g>
            ))}
            {dateColumns.map((col, i) => (
              <g key={i}>
                <line x1={col.x} y1={MARGIN_TOP} x2={col.x} y2={CANVAS_HEIGHT} stroke={col.isMonday ? 'rgba(0,0,0,0.12)' : 'rgba(0,0,0,0.05)'} strokeWidth={col.isMonday ? 1 : 0.5} />
                <text x={col.x + PX_PER_DAY / 2} y={16} fill={col.isWeekend ? 'rgba(0,0,0,0.25)' : col.isMonday ? 'rgba(0,0,0,0.65)' : 'rgba(0,0,0,0.40)'} fontSize={12} textAnchor="middle" fontFamily="monospace" fontWeight={col.isMonday ? 'bold' : 'normal'}>{formatDateLabel(col.date)}</text>
                <text x={col.x + PX_PER_DAY / 2} y={CANVAS_HEIGHT - 4} fill={i === 0 ? 'rgba(14,116,144,0.8)' : col.isWeekend ? 'rgba(0,0,0,0.25)' : col.isMonday ? 'rgba(0,0,0,0.65)' : 'rgba(0,0,0,0.40)'} fontSize={12} textAnchor="middle" fontFamily="monospace" fontWeight={col.isMonday || i === 0 ? 'bold' : 'normal'}>{i === 0 ? `今日 ${formatDateLabel(col.date)}` : formatDateLabel(col.date)}</text>
              </g>
            ))}
          </svg>
            ...
            return (
              <div key={note.id} data-note-id={note.id}
                ...
                <div className="flex items-center justify-between px-2 py-1 border-b border-border">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onRemoveNote(note.id); }} className="text-muted hover:text-red-700 text-xs leading-none">×</button>
                </div>
                {dateBadge && (
      ...
      {editingNote && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/85" onClick={() => setEditingNote(null)}>
          <div className="bg-white border border-primary/30 rounded-xl p-5 w-full max-w-md shadow-2xl shadow-black/60" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
                ...
                <div className="flex gap-1">
                  {(Object.entries(REGION_STYLES) as [StrategyNoteRegion, typeof REGION_STYLES[StrategyNoteRegion]][]).map(([key, s]) => (
                    <button key={key} onClick={() => setEditForm((f) => ({ ...f, region: key }))}
                      className={`flex-1 px-2 py-1.5 rounded text-xs border ${editForm.region === key ? `${s.bg} border-gray-900/30 text-primary` : 'border-primary/10 text-muted'}`}>
                      {s.emoji} {s.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <input type="text" value={editForm.title} onChange={(e) => setEditForm((f) => ({ ...f, title: e.target.value }))}
                  className="w-full px-3 py-2 bg-bg-tertiary border border-primary/20 rounded-lg text-sm text-primary focus:outline-none focus:border-accent-cyan" />
              </div>
              <div>
                <label className="text-xs text-muted block mb-1">説明</label>
                <textarea value={editForm.description} onChange={(e) => setEditForm((f) => ({ ...f, description: e.target.value }))} rows={3}
                  className="w-full px-3 py-2 bg-bg-tertiary border border-primary/20 rounded-lg text-sm text-primary focus:outline-none focus:border-accent-cyan resize-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    <input type="date" value={editForm.date} onChange={(e) => setEditForm((f) => ({ ...f, date: e.target.value }))}
                      className="w-full px-3 py-2 pr-8 bg-bg-tertiary border border-primary/20 rounded-lg text-sm text-primary focus:outline-none focus:border-accent-cyan" />
                    {editForm.date && (
                ...
                <div>
                  ...
                  <input type="text" value={editForm.url} onChange={(e) => setEditForm((f) => ({ ...f, url: e.target.value }))}
                    className="w-full px-3 py-2 bg-bg-tertiary border border-primary/20 rounded-lg text-sm text-primary focus:outline-none focus:border-accent-cyan" placeholder="https://..." />
                </div>
              </div>
