...
export function PortfolioPage() {
  ...
  }, [settings.firebaseProjectId, settings.firebaseApiKey, settings.firebaseAppId]);
  ...
  const handleFetchSectors = async () => {
    if (!settings.geminiApiKey || aggregatedData.length === 0) return;
    setIsAnalyzing(true);
  ...
  const handleFetchIndices = async () => {
    if (!settings.geminiApiKey || history.length === 0) return;
    setIsAnalyzing(true);
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
        ...
        <div className="md:col-span-4 flex flex-col gap-2">
          <button
            ...
            disabled={aggregatedData.length === 0 || isAnalyzing || !settings.geminiApiKey}
          ...
          >
            ...
            {!settings.geminiApiKey && <span className="text-xs text-text-secondary/50 mt-0.5">Gemini APIキー必要</span>}
          </button>
          <button
            onClick={handleFetchIndices}
            disabled={history.length === 0 || isAnalyzing || !settings.geminiApiKey}
            className="flex-1 flex flex-col items-center justify-center p-4 bg-bg-card/70 border border-border text-text-primary rounded-xl hover:bg-bg-card disabled:opacity-30"
          >
